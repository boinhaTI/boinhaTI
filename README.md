# 👋 Oiii, eu sou o Cristofer Dantas!

### 💻 Estudante | Desenvolvedor | Criador de Conteúdo

Sou estudante de programação e tecnologia, apaixonado por desenvolvimento de software e por compartilhar conhecimento.

Atualmente venho focando meus estudos principalmente em **Java, Spring Boot, React, APIs REST, bancos de dados, Docker e arquitetura de sistemas**.

🚀 Gosto de transformar ideias em projetos reais e aprender novas tecnologias colocando a mão no código.

---

## 🧑‍💻 Sobre mim

* 🎓 Estudante de programação e tecnologia
* 💻 Desenvolvedor Full Stack em formação
* ☕ Focado em **Java e Spring Boot**
* ⚛️ Desenvolvendo interfaces com **React**
* 🗄️ Trabalhando com **PostgreSQL**
* 🐳 Estudando e utilizando **Docker**
* ☸️ Estudando **Kubernetes e ambientes de containers**
* 🔐 Interesse em APIs, autenticação, JWT e segurança
* 🌐 Criando projetos utilizando arquitetura moderna e integração entre frontend e backend
* 📚 Sempre buscando aprender novas tecnologias
* 🎥 Criador de conteúdo sobre programação e tecnologia

---

## 🚀 Tecnologias e Ferramentas

### 💻 Linguagens

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="45" height="45" alt="Java"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-plain.svg" width="45" height="45" alt="JavaScript"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-plain.svg" width="45" height="45" alt="TypeScript"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" width="45" height="45" alt="Python"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/csharp/csharp-original.svg" width="45" height="45" alt="C#"/>
</p>

### 🎨 Frontend

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" width="45" height="45" alt="HTML5"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" width="45" height="45" alt="CSS3"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-plain.svg" width="45" height="45" alt="JavaScript"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-plain.svg" width="45" height="45" alt="TypeScript"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" width="45" height="45" alt="React"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/tailwindcss/tailwindcss-original.svg" width="45" height="45" alt="Tailwind CSS"/>
</p>

### ⚙️ Backend

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="45" height="45" alt="Java"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/spring/spring-original.svg" width="45" height="45" alt="Spring"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original.svg" width="45" height="45" alt="Node.js"/>
</p>

**Principais conceitos:**

* Spring Boot
* APIs REST
* Spring Security
* JWT
* Refresh Token
* DTOs
* Validação
* JPA / Hibernate
* Arquitetura em camadas
* Microsserviços
* Mensageria

### 🗄️ Banco de Dados

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original.svg" width="45" height="45" alt="PostgreSQL"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" width="45" height="45" alt="MySQL"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/redis/redis-original.svg" width="45" height="45" alt="Redis"/>
</p>

* PostgreSQL
* MySQL
* Redis
* H2
* SQL
* JPA / Hibernate
* Flyway
* Liquibase

### 🐳 DevOps, Containers e Infraestrutura

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" width="45" height="45" alt="Docker"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/kubernetes/kubernetes-plain.svg" width="45" height="45" alt="Kubernetes"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/githubactions/githubactions-original.svg" width="45" height="45" alt="GitHub Actions"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" width="45" height="45" alt="Linux"/>
</p>

* Docker
* Docker Compose
* Kubernetes
* GitHub Actions
* Linux
* CI/CD
* Containers
* Redes
* Deploy em Cloud

### 📦 Build Tools

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/maven/maven-original.svg" width="45" height="45" alt="Maven"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/gradle/gradle-original.svg" width="45" height="45" alt="Gradle"/>
</p>

* Maven
* Gradle
* Gradle Wrapper (`gradlew`)
* Maven Wrapper

### 🧰 Ferramentas

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" width="45" height="45" alt="Git"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" width="45" height="45" alt="GitHub"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/intellij/intellij-original.svg" width="45" height="45" alt="IntelliJ IDEA"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vscode/vscode-original.svg" width="45" height="45" alt="VS Code"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postman/postman-original.svg" width="45" height="45" alt="Postman"/>
</p>

* Git
* GitHub
* IntelliJ IDEA
* Visual Studio Code
* Postman
* DBeaver
* Docker Desktop

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=boinhaTI&show_icons=true&theme=transparent&include_all_commits=true&count_private=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=boinhaTI&layout=compact&langs_count=16&theme=transparent"/>

</div>

---

## 🚀 Projetos em destaque

### 💧 ÁguaControl PRO

Sistema para gerenciamento de abastecimento de água, desenvolvido como projeto acadêmico e prático.

**Tecnologias:**

`Java` `Spring Boot` `React` `PostgreSQL` `JWT` `Docker` `REST API`

---

### 🍔 Delivery App

Projeto de estudo baseado em uma arquitetura de microsserviços para gerenciamento de clientes, vendedores e produtos.

**Tecnologias:**

`Java` `Spring Boot` `React` `PostgreSQL` `Redis` `Docker` `Docker Compose`

---

### 🏦 BankLite

Projeto de estudo envolvendo arquitetura distribuída e comunicação assíncrona entre serviços.

**Tecnologias:**

`Java` `Spring Boot` `Kafka` `PostgreSQL` `Docker` `Microsserviços`

---

## 📚 Atualmente estudando

```text
Java
Spring Boot
Spring Security
JWT
Microsserviços
Apache Kafka
Redis
Docker
Kubernetes
CI/CD
PostgreSQL
React
TypeScript
Arquitetura de Software
```

---

## 📈 Objetivos

Meu objetivo é evoluir cada vez mais como desenvolvedor, construindo projetos reais e aprofundando meus conhecimentos em:

* 🏗️ Arquitetura de Software
* ☕ Java / Spring Boot
* ⚛️ React / TypeScript
* 🔐 Segurança de APIs
* 🧩 Microsserviços
* 📨 Mensageria
* 🐳 Docker
* ☸️ Kubernetes
* ☁️ Cloud
* 🔄 CI/CD
* 🗄️ Banco de Dados

---

## 🌎 Onde me encontrar

<p align="left">

<a href="https://github.com/boinhaTI">
  <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/cristofer-dantas-15b757298/">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://www.instagram.com/boinha_graf/">
  <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
</a>

<a href="https://api.whatsapp.com/send/?phone=5588997413745&text&type=phone_number&app_absent=0">
  <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"/>
</a>

</p>

---

<div align="center">

### 💻 "Transformando ideias em código."

⭐ Se algum projeto meu te ajudou, considere deixar uma estrela!

</div>
